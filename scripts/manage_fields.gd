extends Control
## Gestión de escenarios: lista, alta con foto, borrado e importación.
...
func _ready() -> void:
	...
	title.add_theme_color_override("font_color", Color(0.7, 1.0, 0.75))
	top.add_child(title)
	...
	flabel.text = "Añadir escenario nuevo"
	flabel.add_theme_font_size_override("font_size", 28)
	form.add_child(flabel)

	_name_edit = LineEdit.new()
	_name_edit.placeholder_text = "Nombre del escenario"
	...
	_photo_btn.text = "⛳  Elegir foto del escenario"
	...
	_preview.custom_minimum_size = Vector2(112, 63)
	...
	imp_label.text = "También puedes copiar fotos a la carpeta de importación (una imagen = un escenario) y pulsar el botón:"
	...
	imp_path.text = FieldDB.import_folder_os_path()
	...
	ht_title.text = "🎵 Música del descanso"
	ht_title.add_theme_font_size_override("font_size", 24)
	form.add_child(ht_title)
	_ht_status = Label.new()
	_ht_status.add_theme_font_size_override("font_size", 18)
	_ht_status.add_theme_color_override("font_color", Color(0.7, 0.9, 1.0))
	form.add_child(_ht_status)
	var ht_row := HBoxContainer.new()
	ht_row.add_theme_constant_override("separation", 12)
	form.add_child(ht_row)
	var ht_pick := Button.new()
	ht_pick.text = "🎵  Elegir audio"
	ht_pick.add_theme_font_size_override("font_size", 20)
	ht_pick.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	ht_pick.pressed.connect(_pick_halftime_audio)
	ht_row.add_child(ht_pick)
	var ht_clear := Button.new()
	ht_clear.text = "🗑  Quitar"
	ht_clear.add_theme_font_size_override("font_size", 20)
	ht_clear.pressed.connect(func():
		GameState.clear_halftime_audio()
		_update_ht_status())
	ht_row.add_child(ht_clear)
	_update_ht_status()

	_refresh_list()

func _update_ht_status() -> void:
	var p: String = GameState.halftime_audio_path()
	if p == "":
		_ht_status.text = "Sonará la música de serie (piti time)."
	else:
		_ht_status.text = "Audio guardado (%s). Sonará durante el descanso." % p.get_extension().to_upper()

func _pick_halftime_audio() -> void:
	_pick_file(PackedStringArray(["*.ogg, *.mp3, *.wav ; Audio"]), func(path):
		var err: String = GameState.set_halftime_audio(path)
		if err == "":
			_status.add_theme_color_override("font_color", Color(0.5, 1, 0.5))
			_status.text = "✔ Música del descanso guardada."
		else:
...
func _refresh_list() -> void:
	...
	for f in FieldDB.fields:
		...
		if f.kind == "photo" and f.texture != null:
			...
			tr.custom_minimum_size = Vector2(96, 54)
			tr.clip_contents = true
			hb.add_child(tr)
		else:
			var holder := Control.new()
			holder.custom_minimum_size = Vector2(96, 54)
			holder.clip_contents = true
			var art := FieldArt.new()
			art.setup(f)
			art.scale = Vector2.ONE * (96.0 / 1280.0)
...
func _pick_photo() -> void:
	_pick_file(PackedStringArray(["*.png, *.jpg, *.jpeg, *.webp ; Imágenes"]), func(path):
		...
		var img: Image = PlayerDB._load_image_any(path)
		if img != null:
...
func _save() -> void:
	var err: String = FieldDB.add_field(_name_edit.text, _photo_path)
	if err == "":
		_status.add_theme_color_override("font_color", Color(0.5, 1, 0.5))
		_status.text = "✔ Escenario \"%s\" guardado." % _name_edit.text.strip_edges()
		_name_edit.text = ""
		_photo_path = ""
		_photo_btn.text = "⛳  Elegir foto del escenario"
